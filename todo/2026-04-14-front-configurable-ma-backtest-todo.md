...
- 股票代码切换直接复用研究页现有 selector，避免第一版再新增一套自由输入、校验和补全交互。
- 用固定模板先把端到端链路跑通，后续再扩到 EMA、双均线、止损止盈、多个规则组合会更稳。

## 第一版范围

- 仅支持单标的、单策略、只做多
- 股票代码切换明确复用现有 research selector，不新增自由输入框
- 仅支持一条均线
...
- [ ] 股票代码切换明确复用现有 `selectedSymbol` 与现有 selector，不新增单独 symbol 输入框
...
- 前端能发起 `1d` 规则回测
- 股票切换沿用现有 research 交互，不额外增加第二套 symbol 控件
...
- [ ] 新增一个专门的前端表单区域
- [ ] 明确该区域不重复渲染 symbol selector，直接读取研究页顶部当前已选股票代码
- [ ] 字段建议：
  - 均线周期
  - 低于均线买入比例
  - 高于均线卖出比例
...
- 用户切换股票代码时，规则回测默认跟随研究页当前已选 symbol
- 第一眼就能理解每个输入框是干什么的
