...
- 研究与回测：
  `apps/research` 暴露最小事件驱动回测 runner，复用同一套 strategy、order plan、paper execution 和 portfolio ledger 语义。
- 可观测性：
...
当前回测能力以 Python API 形式提供，而不是独立 CLI。典型入口是：

- `apps.research.build_default_backtest_runner(...)`
- `apps.research.ResearchBacktestRunner.run(...)`

它复用了生产链路中的策略、order plan、paper execution 与持仓账本语义，适合做最小一致性验证。一个完整用法可以参考：
...
3. `make test-e2e`
4. 启动 `make trader`
5. 用 API 访问 `/v1/status`、`/v1/positions`、`/v1/orders/active`
6. 通过 `/v1/controls/*` 试运行 pause、kill switch、cancel-all
